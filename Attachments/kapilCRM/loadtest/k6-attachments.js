...
const runQuery = (endpoint, params = "") => {
  const res = http.get(`${API_BASE}/${endpoint}${params}`, { headers });
  check(res, {
  ...
  if (res.status !== 200 && errorSamples < MAX_ERROR_SAMPLES) {
    ...
    console.warn(`k6-attachments ${endpoint} failed: status=${res.status} body=${snippet}`);
  }
};

export default function () {
  runQuery("attachments/getFolders");
  runQuery("attachments/getDocuments", "?kpiId=123");
  runQuery("attachments/upload");
  sleep(1);
}