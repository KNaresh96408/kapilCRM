...
import {
  ...
  MAROON,
  updateVendorTotalsOnMaterialPayment,
} from './purchaseHelpers';

const MaterialPOPayment = ({ rows = [], currentUserRole = '' }) => {
  ...
  const savePayment = async () => {
    ...
    try {
      ...
      if (file) {
        const storageRef = ref(storage, `materialReceipts/${selected.poNumber}.pdf`);
        await uploadBytes(storageRef, file, { contentType: file.type || 'application/pdf' });
        receiptUrl = await getDownloadURL(storageRef);
      }

      await updateVendorTotalsOnMaterialPayment({
        poId: selected.id,
      ...
      await createNotificationIfNotExists({
        dedupeKey: `po_payment_update_${selected.id}_${Number(paidAmount || 0)}`,
...
export default MaterialPOPayment;
